...
Value mnbudget(const Array& params, bool fHelp)
{
    ...
    if(strCommand == "prepare")
    {
        ...
        if (params.size() != 7)
            throw runtime_error("Correct usage is 'mnbudget prepare proposal-name url payment_count block_start katana_address monthly_payment_katana'");
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Katana address");

        // Parse Katana address
    ...
    if(strCommand == "submit")
    {
        ...
        if (params.size() != 8)
            throw runtime_error("Correct usage is 'mnbudget submit proposal-name url payment_count block_start katana_address monthly_payment_katana fee_tx'");
